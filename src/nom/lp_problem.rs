...
use crate::nom::{
    decoder::{
        constraint::{parse_cons, parse_cons_header},
...
impl<'a> TryFrom<&'a str> for LPProblem<'a> {
    ...
    fn try_from(input: &'a str) -> Result<Self, Self::Error> {
        let (input, (comments, sense, obj_section, _)) = tuple((
            parse_comments,
            parse_sense,
        ...
        let objectives = match parse_objectives(obj_section) {
            Ok((_, objs)) => objs,
            Err(_) => Vec::new(),
        };

        // Parse the constraints
        let (_remaining, constraints) = parse_cons(input)?;

        // Parse Variable Bounds (Integer, General, Bounded, Free, Semi-continuous and SOS)
        // // TODO

        Ok(Self { name: comments, sense, objectives, constraints, variables: vec![] })
    }
}

#[cfg(test)]
mod test {
    use crate::nom::lp_problem::LPProblem;
...
c3:  2 x2 + 3 x4 <= 25";
    ...
    fn test_sections_so_far() {
        ...
        insta::assert_debug_snapshot!(&problem);
    }
    ...
    fn test_serialization_lifecycle() {
        ...
        insta::assert_debug_snapshot!(&serialized_problem);

        // Deserialize
        let problem: LPProblem<'_> = serde_json::from_str(&serialized_problem).expect("test case not to fail");
        insta::assert_debug_snapshot!(&problem);
    }
}
