...
/// # Errors
///
/// Returns an `LpParseError::IoError` if the file cannot be read or is not valid UTF-8.
pub fn parse_file(path: &Path) -> LpResult<String> {
    std::fs::read_to_string(path).map_err(|e| LpParseError::io_error(format!("Failed to read file '{}': {}", path.display(), e)))
}
